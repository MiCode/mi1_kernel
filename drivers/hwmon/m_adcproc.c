		      ...
		      struct adc_chan_result *adc_chan_result)
{
	...
	adc_chan_result->measurement =
	    (int64_t) rawfromoffset * chan_properties->adc_graph->dx *
	    chan_properties->gain_denominator;
			 ...
			 struct adc_chan_result *adc_chan_result)
{
	...
	return adc_map_linear(adcmap_batttherm,
			      sizeof(adcmap_batttherm) /
			      sizeof(adcmap_batttherm[0]),
			...
			struct adc_chan_result *adc_chan_result)
{
	scale_default(adc_code, adc_properties, chan_properties,
		      adc_chan_result);
	/* convert mV ---> degC using the table */
	return adc_map_linear(adcmap_msmtherm,
			      sizeof(adcmap_msmtherm) /
			      sizeof(adcmap_msmtherm[0]),
			      adc_chan_result->physical,
			      &adc_chan_result->physical);
}
		     ...
		     struct adc_chan_result *adc_chan_result)
{
	int32_t offset = chan_properties->adc_graph->offset,
	    ...
	    dx = chan_properties->adc_graph->dx, fullscale_calibrated_adc_code;
	...
	if ((dy == 0) || (dx == 0) || (offset >= fullscale_calibrated_adc_code)) {
		return -EINVAL;
	} else {
		...
		adc_map_linear(adcmap_ntcg104ef104fb,
			       sizeof(adcmap_ntcg104ef104fb) /
			       sizeof(adcmap_ntcg104ef104fb[0]),
