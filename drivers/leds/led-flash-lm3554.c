...
struct lm3554_data *g_lm3554_data;

static int lm3554_i2c_rxdata(struct i2c_client *client, unsigned char *rxdata,
			     int length)
{
	uint8_t loop_i;
	struct i2c_msg msgs[] = {
...
static int32_t lm3554_i2c_txdata(struct i2c_client *client,
...
EXPORT_SYMBOL(lm3554_flashlight_control);
				  ...
				  const char *buf, size_t count)
{
	...
	err = lm3554_write_reg(torch_data, LM3554_TORCH_BRIGHTNESS, val);
	if (err) {
			...
			const struct i2c_device_id *id)
{
	...
	pr_info("lm3554 gpio%d :%d\n", pdata->hwen_gpio,
		gpio_get_value_cansleep(pdata->hwen_gpio));
	...
	err = device_create_file(torch_data->led_dev.dev, &dev_attr_strobe_err);
	if (err < 0) {
		err = -ENODEV;
		pr_err("%s:File device creation failed: %d\n", __func__, err);
		goto error6;
