...
static int wl_iw_set_btcoex_dhcp(struct net_device *dev,
...
static int
dev_wlc_intvar_get_reg(struct net_device *dev,
		       char *name, uint reg, int *retval)
{
	union {
	...
	uint len;
	len =
	    bcm_mkiovar(name, (char *)(&reg), sizeof(reg), (char *)(&var),
			sizeof(var.buf));
	ASSERT(len);
...
static int
dev_wlc_intvar_set_reg(struct net_device *dev,
		       char *name, char *addr, char *val)
{
	char reg_addr[8];
...
#if WIRELESS_EXT > 12
static int
wl_iw_set_active_scan(struct net_device *dev,
		      struct iw_request_info *info,
		      union iwreq_data *wrqu, char *extra)
{
	int as = 0;
...
static int
wl_iw_set_passive_scan(struct net_device *dev,
		       struct iw_request_info *info,
		       union iwreq_data *wrqu, char *extra)
{
	int ps = 1;
	...
	if (g_iscan->iscan_state == ISCAN_STATE_IDLE) {
		...
		if (g_scan_specified_ssid == 0) {
			error =
			    dev_wlc_ioctl(dev, WLC_SET_PASSIVE_SCAN, &ps,
					  sizeof(ps));
		}
#if defined(WL_IW_USE_ISCAN)
	} else
...
static int
wl_iw_set_txpower(struct net_device *dev,
		  struct iw_request_info *info,
		  union iwreq_data *wrqu, char *extra)
{
	int error = 0;
...
static int
wl_iw_get_macaddr(struct net_device *dev,
		  struct iw_request_info *info,
		  union iwreq_data *wrqu, char *extra)
{
	int error;
	...
	p += snprintf(p, MAX_WX_STRING,
		      ...
		      id->octet[1], id->octet[2], id->octet[3], id->octet[4],
		      id->octet[5]);
	wrqu->data.length = p - extra + 1;
...
static int
wl_iw_set_country(struct net_device *dev,
		  struct iw_request_info *info,
		  union iwreq_data *wrqu, char *extra)
{
	...
	if (country_offset != 0) {
		...
		get_customized_country_code((char *)&cspec.country_abbrev,
					    &cspec);
...
static int
wl_iw_set_power_mode(struct net_device *dev,
		     struct iw_request_info *info,
		     union iwreq_data *wrqu, char *extra)
{
	int error = 0;
	...
	if (strnicmp((char *)&powermode_val, "1", strlen("1")) == 0) {
		...
		WL_TRACE_COEX(("%s: DHCP done for:%d ms, restored pm:%d\n",
			       __FUNCTION__,
			       (g_bt->ts_dhcp_ok - g_bt->ts_dhcp_start), pm));
#endif
...
static int
wl_iw_get_power_mode(struct net_device *dev,
		     struct iw_request_info *info,
		     union iwreq_data *wrqu, char *extra)
{
	int error = 0;
...
static int
wl_iw_set_btcoex_dhcp(struct net_device *dev,
		      struct iw_request_info *info,
		      union iwreq_data *wrqu, char *extra)
{
	int error = 0;
...
static int
wl_iw_set_suspend_opt(struct net_device *dev,
		      struct iw_request_info *info,
		      union iwreq_data *wrqu, char *extra)
{
	int suspend_flag;
...
static int
wl_iw_set_suspend_mode(struct net_device *dev,
		       struct iw_request_info *info,
		       union iwreq_data *wrqu, char *extra)
{
	int ret = 0;
...
static int
wl_iw_get_link_speed(struct net_device *dev,
		     struct iw_request_info *info,
		     union iwreq_data *wrqu, char *extra)
{
	int error = 0;
	...
	if (g_onoff == G_WLAN_SET_ON) {
		error =
		    dev_wlc_ioctl(dev, WLC_GET_RATE, &link_speed,
				  sizeof(link_speed));
		link_speed *= 500000;
	}
...
static int
wl_iw_get_dtim_skip(struct net_device *dev,
		    struct iw_request_info *info,
		    union iwreq_data *wrqu, char *extra)
{
	int error = -1;
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		if ((error = dev_wlc_ioctl(dev, WLC_GET_VAR,
			...
			p += snprintf(p, MAX_WX_STRING, "Dtim_skip %d",
				      iovbuf[0]);
...
static int
wl_iw_set_dtim_skip(struct net_device *dev,
		    struct iw_request_info *info,
		    union iwreq_data *wrqu, char *extra)
{
	int error = -1;
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		bcn_li_dtim =
		    htod32((uint) *(extra + strlen(DTIM_SKIP_SET_CMD) + 1) -
			   '0');
		...
		if ((bcn_li_dtim >= 0) || ((bcn_li_dtim <= 5))) {
			...
			if ((error = dev_wlc_ioctl(dev, WLC_SET_VAR,
				...
				WL_ERROR(("%s: set dtim_skip %d failed code %d\n", __FUNCTION__, bcn_li_dtim, error));
		} else
			WL_ERROR(("%s Incorrect dtim_skip setting %d, ignored\n", __FUNCTION__, bcn_li_dtim));
	}
...
static int
wl_iw_get_band(struct net_device *dev,
	       struct iw_request_info *info,
	       union iwreq_data *wrqu, char *extra)
{
	int error = -1;
...
static int
wl_iw_set_band(struct net_device *dev,
	       struct iw_request_info *info,
	       union iwreq_data *wrqu, char *extra)
{
	int error = -1;
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		band =
		    htod32((uint) *(extra + strlen(BAND_SET_CMD) + 1) - '0');
...
static int
wl_iw_set_pno_reset(struct net_device *dev,
		    struct iw_request_info *info,
		    union iwreq_data *wrqu, char *extra)
{
	int error = -1;
...
static int
wl_iw_set_pno_enable(struct net_device *dev,
		     struct iw_request_info *info,
		     union iwreq_data *wrqu, char *extra)
{
	int error = -1;
	...
	net_os_wake_lock(dev);
	pfn_enabled =
	    htod32((uint) *(extra + strlen(PNOENABLE_SET_CMD) + 1) - '0');
...
static int
wl_iw_set_pno_set(struct net_device *dev,
		  struct iw_request_info *info,
		  union iwreq_data *wrqu, char *extra)
{
	int res = -1;
	...
	net_os_wake_lock(dev);
	WL_ERROR(("\n### %s: info->cmd:%x, info->flags:%x, u.data=0x%p, u.len=%d\n", __FUNCTION__, info->cmd, info->flags, wrqu->data.pointer, wrqu->data.length));
	...
	if (wrqu->data.length < (strlen(PNOSETUP_SET_CMD) + sizeof(cmd_tlv_t))) {
		WL_ERROR(("%s argument=%d less %d\n", __FUNCTION__,
			  wrqu->data.length,
			  (int)(strlen(PNOSETUP_SET_CMD) + sizeof(cmd_tlv_t))));
		goto exit_proc;
	}
#ifdef PNO_SET_DEBUG
	...
	res =
	    dhd_dev_pno_set(dev, ssids_local, nssid, pno_time, pno_repeat,
			    pno_freq_expo_max);
...
static int
wl_iw_set_pno_setadd(struct net_device *dev,
		     struct iw_request_info *info,
		     union iwreq_data *wrqu, char *extra)
{
	int ret = -1;
...
static int
wl_iw_get_rssi(struct net_device *dev,
	       struct iw_request_info *info,
	       union iwreq_data *wrqu, char *extra)
{
	static int rssi;
	...
	if (g_onoff == G_WLAN_SET_ON) {
		error =
		    dev_wlc_ioctl(dev, WLC_GET_RSSI, &scb_val,
				  sizeof(scb_val_t));
		if (error) {
			WL_ERROR(("%s: Fails %d\n", __FUNCTION__, error));
			net_os_wake_unlock(dev);
		...
		if (!error) {
			...
			wl_format_ssid(ssidbuf, ssid.SSID,
				       dtoh32(ssid.SSID_len));
		}
	}
...
static int
wl_iw_control_wl_off(struct net_device *dev, struct iw_request_info *info)
{
	wl_iw_t *iw;
	int ret = 0;
...
static int
wl_iw_control_wl_on(struct net_device *dev, struct iw_request_info *info)
{
	int ret = 0;
...
static int get_parameter_from_string(char **str_ptr, const char *token,
				     int param_type, void *dst,
				     int param_max_len);
...
static int hstr_2_buf(const char *txt, u8 *buf, int len)
{
	int i;
...
init_ap_profile_from_string(char *param_str, struct ap_profile *ap_cfg)
{
	...
	ret =
	    get_parameter_from_string(&str_ptr, "SSID=", PTYPE_STRING,
				      ap_cfg->ssid, SSID_LEN);

	ret |=
	    get_parameter_from_string(&str_ptr, "SEC=", PTYPE_STRING,
				      ap_cfg->sec, SEC_LEN);

	ret |=
	    get_parameter_from_string(&str_ptr, "KEY=", PTYPE_STRING,
				      ap_cfg->key, KEY_LEN);

	ret |=
	    get_parameter_from_string(&str_ptr, "CHANNEL=", PTYPE_INTDEC,
				      &ap_cfg->channel, 5);

	get_parameter_from_string(&str_ptr, "PREAMBLE=", PTYPE_INTDEC,
				  &ap_cfg->preamble, 5);

	get_parameter_from_string(&str_ptr, "MAX_SCB=", PTYPE_INTDEC,
				  &ap_cfg->max_scb, 5);
		     ...
		     union iwreq_data *wrqu, char *ext)
{
	...
	if (wrqu->data.length != 0) {
		...
		if (copy_from_user
		    (extra, wrqu->data.pointer, wrqu->data.length)) {
			kfree(extra);
			return -EFAULT;
		}
...
static int iwpriv_get_assoc_list(struct net_device *dev,
				 struct iw_request_info *info,
				 union iwreq_data *p_iwrq, char *extra)
{
	int i, ret = 0;
	char mac_buf[256];
	...
	WL_TRACE(("\n %s: IWPRIV IOCTL: cmd:%hx, flags:%hx, extra:%p, iwp.len:%d," "iwp.len:%p, iwp.flags:%x  \n", __FUNCTION__, info->cmd, info->flags, extra, p_iwrq->data.length, p_iwrq->data.pointer, p_iwrq->data.flags));
				      ...
				      union iwreq_data *wrqu, char *ext)
{
	...
	if (wrqu->data.length != 0) {
		...
		if (copy_from_user
		    (cmd_buf, wrqu->data.pointer, wrqu->data.length)) {
			return -EFAULT;
		}

		if (get_parameter_from_string(&str_ptr,
					      "MAC=", PTYPE_STR_HEX, sta_mac,
					      12) == 0) {
...
static void wl_iw_set_ss_cache_timer_flag(void)
{
	g_ss_cache_ctrl.m_timer_expired = 1;
	WL_TRACE(("%s called\n", __FUNCTION__));
}

static int wl_iw_init_ss_cache_ctrl(void)
{
	WL_TRACE(("%s :\n", __FUNCTION__));
	...
	g_ss_cache_ctrl.m_timer =
	    kmalloc(sizeof(struct timer_list), GFP_KERNEL);
	if (!g_ss_cache_ctrl.m_timer) {
		return -ENOMEM;
	}
	g_ss_cache_ctrl.m_timer->function =
	    (void *)wl_iw_set_ss_cache_timer_flag;
...
static void wl_iw_release_ss_cache_ctrl(void)
{
	WL_TRACE(("%s :\n", __FUNCTION__));
	wl_iw_free_ss_cache();
	wl_iw_run_ss_cache_timer(0);
...
static void wl_iw_reset_ss_cache(void)
{
...
static int wl_iw_add_bss_to_ss_cache(wl_scan_results_t *ss_list)
{
	...
	for (i = 0; i < ss_list->count; i++) {
		...
		leaf =
		    kmalloc(bi->length + WLC_IW_SS_CACHE_CTRL_FIELD_MAXLEN,
			    GFP_KERNEL);
		if (!leaf) {
...
wl_iw_merge_scan_cache(struct iw_request_info *info, char *extra,
		       uint buflen_from_user, __u16 *merged_len)
{
...
static int wl_iw_delete_bss_from_ss_cache(void *addr)
{
...
int wl_iw_iscan_set_scan_broadcast_prep(struct net_device *dev, uint flag)
{
	...
	dev_wlc_ioctl(dev, WLC_SET_PASSIVE_SCAN, &iscan->scan_flag,
		      sizeof(iscan->scan_flag));
		     ...
		     union iwreq_data *wrqu, char *extra)
{
	...
	if (wrqu->data.length == sizeof(struct iw_scan_req)) {
		if (wrqu->data.flags & IW_SCAN_THIS_ESSID) {
			...
			dev_wlc_ioctl(dev, WLC_SET_PASSIVE_SCAN, &as,
				      sizeof(as));
			wl_iw_set_event_mask(dev);
	       ...
	       struct iw_request_info *info, struct iw_point *dwrq, char *extra)
{
	...
	len_ret =
	    (__u16) wl_iw_get_scan_prep(list_merge, info, extra,
					buflen_from_user);
#endif
	DHD_OS_MUTEX_UNLOCK(&wl_cache_lock);
	...
	wl_iw_merge_scan_cache(info, extra + len_ret,
			       buflen_from_user - len_ret, &merged_len);
	len_ret += merged_len;
	wl_iw_run_ss_cache_timer(0);
		     ...
		     struct iw_point *dwrq, char *extra)
{
	...
	if (g_scan_specified_ssid) {
		...
		if (g_ss_cache_ctrl.m_link_down) {
			...
			wl_iw_delete_bss_from_ss_cache(g_ss_cache_ctrl.
						       m_active_bssid);
		}
		if (g_ss_cache_ctrl.m_prev_scan_mode
		    || g_ss_cache_ctrl.m_cons_br_scan_cnt > 4) {
			g_ss_cache_ctrl.m_cons_br_scan_cnt = 0;
	...
	wl_iw_merge_scan_cache(info, event, buflen_from_user - dwrq->length,
			       &merged_len);
...
#ifdef CONFIG_PRESCANNED
static int check_prescan(wl_join_params_t *join_params, int *join_params_size)
{
	int cnt = 0;
	int indx = 0;
	...
	if (!cnt) {
		...
		for (; node;) {
			if (!memcmp
			    (&node->bss_info->SSID, join_params->ssid.SSID,
			     join_params->ssid.SSID_len)) {
				memcpy(&join_params->params.chanspec_list[cnt],
				       &node->bss_info->chanspec,
				       sizeof(chanspec_t));
		 ...
		 struct iw_point *dwrq, char *extra)
{
	...
	WL_TRACE(("%s: SIOCSIWENCODE index %d, len %d, flags %04x (%s%s%s%s%s)\n", dev->name, dwrq->flags & IW_ENCODE_INDEX, dwrq->length, dwrq->flags, dwrq->flags & IW_ENCODE_NOKEY ? "NOKEY" : "", dwrq->flags & IW_ENCODE_DISABLED ? " DISABLED" : "", dwrq->flags & IW_ENCODE_RESTRICTED ? " RESTRICTED" : "", dwrq->flags & IW_ENCODE_OPEN ? " OPEN" : "", dwrq->flags & IW_ENCODE_TEMP ? " TEMP" : ""));
		...
		struct iw_param *vwrq, char *extra)
{
	...
	if (iwpmksa->cmd == IW_PMKSA_FLUSH) {
	...
	else if (iwpmksa->cmd == IW_PMKSA_REMOVE) {
		{
			...
			WL_WSEC(("wl_iw_set_pmksa,IW_PMKSA_REMOVE - PMKID: %s = ", bcm_ether_ntoa(&pmkidptr->pmkid[0].BSSID, eabuf)));
		...
		for (i = 0; i < pmkid_list.pmkids.npmkid; i++)
			if (!bcmp
			    (&iwpmksa->bssid.sa_data[0], &pmkid_array[i].BSSID,
			     ETHER_ADDR_LEN))
				break;
		...
		if ((pmkid_list.pmkids.npmkid > 0)
			...
			for (; i < (pmkid_list.pmkids.npmkid - 1); i++) {
				bcopy(&pmkid_array[i + 1].BSSID,
				      &pmkid_array[i].BSSID, ETHER_ADDR_LEN);
				bcopy(&pmkid_array[i + 1].PMKID,
				      &pmkid_array[i].PMKID, WPA2_PMKID_LEN);
			}
...
static uint32 wl_iw_create_wpaauth_wsec(struct net_device *dev)
{
	wl_iw_t *iw = NETDEV_PRIV(dev);
	uint32 wsec;
		  ...
		  struct iw_param *vwrq, char *extra)
{
	...
	WL_TRACE(("%s: SIOCSIWAUTH, %s(%d), paramval = 0x%0x\n",
		  ...
		  paramid ==
		  IW_AUTH_CIPHER_PAIRWISE ? "IW_AUTH_CIPHER_PAIRWISE" : paramid
		  == IW_AUTH_CIPHER_GROUP ? "IW_AUTH_CIPHER_GROUP" : paramid ==
		  IW_AUTH_KEY_MGMT ? "IW_AUTH_KEY_MGMT" : paramid ==
		  IW_AUTH_TKIP_COUNTERMEASURES ? "IW_AUTH_TKIP_COUNTERMEASURES"
		  : paramid ==
		  IW_AUTH_DROP_UNENCRYPTED ? "IW_AUTH_DROP_UNENCRYPTED" :
		  paramid ==
		  IW_AUTH_80211_AUTH_ALG ? "IW_AUTH_80211_AUTH_ALG" : paramid ==
		  IW_AUTH_WPA_ENABLED ? "IW_AUTH_WPA_ENABLED" : paramid ==
		  IW_AUTH_RX_UNENCRYPTED_EAPOL ? "IW_AUTH_RX_UNENCRYPTED_EAPOL"
		  : paramid ==
		  IW_AUTH_ROAMING_CONTROL ? "IW_AUTH_ROAMING_CONTROL" : paramid
		  ==
		  IW_AUTH_PRIVACY_INVOKED ? "IW_AUTH_PRIVACY_INVOKED" :
		  "UNKNOWN", paramid, paramval));
	...
	case IW_AUTH_DROP_UNENCRYPTED:
		if ((error =
		     dev_wlc_intvar_set(dev, "wsec_restrict", paramval)))
			return error;
		break;
...
static int wl_iw_parse_wep(char *keystr, wl_wsec_key_t *key)
{
	char hex[] = "XX";
...
extern void pbkdf2_sha1(const char *passphrase, const char *ssid,
			size_t ssid_len, int iterations, u8 *buf,
			size_t buflen);
...
#ifndef AP_ONLY
static int
wl_bssiovar_mkbuf(const char *iovar,
		  int bssidx,
		  void *param,
...
static int
wl_iw_combined_scan_set(struct net_device *dev, wlc_ssid_t *ssids_local,
			int nssid, int nchan)
{
	int params_size =
	...
	if (nssid > 0) {
		...
		p = (char *)iscan->iscan_ex_params_p->params.channel_list +
		    nchan * sizeof(uint16);
	}
	...
	nssid = (uint)
	    ((iscan->iscan_ex_params_p->params.
	      channel_num >> WL_SCAN_PARAMS_NSSID_SHIFT) &
	     WL_SCAN_PARAMS_COUNT_MASK);

	params_size =
	    (int)(p - (char *)iscan->iscan_ex_params_p +
		 ...
		 union iwreq_data *wrqu, char *ext)
{
	...
	get_parameter_from_string(&str_ptr, GET_ACTIVE_ASSOC_DWELL,
				  PTYPE_INTDEC,
				  &iscan->iscan_ex_params_p->params.active_time,
				  4);

	get_parameter_from_string(&str_ptr, GET_PASSIVE_ASSOC_DWELL,
				  PTYPE_INTDEC,
				  &iscan->iscan_ex_params_p->params.
				  passive_time, 4);
...
static int
wl_iw_set_cscan(struct net_device *dev,
		struct iw_request_info *info,
		union iwreq_data *wrqu, char *extra)
{
	int res = -1;
	...
	if (wrqu->data.length < (strlen(CSCAN_COMMAND) + sizeof(cscan_tlv_t))) {
		WL_ERROR(("%s argument=%d  less %d\n", __FUNCTION__,
			  wrqu->data.length,
			  (int)(strlen(CSCAN_COMMAND) + sizeof(cscan_tlv_t))));
		return -1;
	}
#ifdef TLV_DEBUG
	...
	if ((cscan_tlv_temp->prefix == CSCAN_TLV_PREFIX) &&
		...
		if ((nssid = wl_iw_parse_ssid_list_tlv(&str_ptr, ssids_local,
			...
			memset(iscan->iscan_ex_params_p, 0,
			       iscan->iscan_ex_param_size);

			wl_iw_iscan_prep(&iscan->iscan_ex_params_p->params,
					 NULL);
			iscan->iscan_ex_params_p->version =
			    htod32(ISCAN_REQ_VERSION);
			iscan->iscan_ex_params_p->action =
			...
			while (tlv_size_left > 0) {
				...
				case CSCAN_TLV_TYPE_CHANNEL_IE:
					...
					if ((nchan =
					     wl_iw_parse_channel_list_tlv
					     (&str_ptr,
					      &iscan->iscan_ex_params_p->params.
					      channel_list[0], WL_NUMCHANNELS,
					      &tlv_size_left)) == -1) {
						WL_ERROR(("%s missing channel list\n", __FUNCTION__));
						goto exit_proc;
					}
...
#ifdef CONFIG_WPS2
static int
wl_iw_del_wps_probe_req_ie(struct net_device *dev,
			   struct iw_request_info *info,
			   union iwreq_data *wrqu, char *extra)
{
	int ret;
	...
	if (g_wps_probe_req_ie) {
		ie_delbuf =
		    (vndr_ie_setbuf_t *) (g_wps_probe_req_ie +
					  strlen("vndr_ie "));
		strncpy(ie_delbuf->cmd, "del", 3);
		ie_delbuf->cmd[3] = '\0';

		ret =
		    dev_wlc_ioctl(dev, WLC_SET_VAR, g_wps_probe_req_ie,
				  g_wps_probe_req_ie_len);
		if (ret) {
...
static int
wl_iw_add_wps_probe_req_ie(struct net_device *dev,
			   struct iw_request_info *info,
			   union iwreq_data *wrqu, char *extra)
{
	char *str_ptr = NULL;
	...
	if (!g_wps_probe_req_ie) {
		...
		iecount = htod32(1);
		memcpy((void *)&ie_setbuf->vndr_ie_buffer.iecount, &iecount,
		       sizeof(int));

		pktflag = 0x10;
		memcpy((void *)&ie_setbuf->vndr_ie_buffer.vndr_ie_list[0].
		       pktflag, &pktflag, sizeof(uint32));

		memcpy((void *)&ie_setbuf->vndr_ie_buffer.vndr_ie_list[0].
		       vndr_ie_data, str_ptr, datalen);
		...
		iolen =
		    bcm_mkiovar("vndr_ie", (char *)ie_setbuf, buflen, bufptr,
				total_len);
		if (iolen == 0) {
...
get_softap_auto_channel(struct net_device *dev, struct ap_profile *ap)
{
	...
	iolen = wl_bssiovar_mkbuf("ssid", bsscfg_index, (char *)(&null_ssid),
				  null_ssid.SSID_len + 4, buf, sizeof(buf),
				  &mkvar_err);
	ASSERT(iolen);
	...
	ret =
	    dev_wlc_ioctl(dev, WLC_START_CHANNEL_SEL, &request,
			  sizeof(request));
	if (ret < 0) {
...
get_channel_retry:
	...
	if (ret < 0 || dtoh32(chosen) == 0) {
		if (retry++ < 15) {
			...
			if (ret < 0) {
				WL_ERROR(("can't get auto channel sel, err = %d, " "chosen = 0x%04X\n", ret, (uint16) chosen));
				goto fail;
			} else {
...
static int set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	if (ap_cfg_running == FALSE) {
		...
		apsta_var = 0;
		if ((res =
		     dev_wlc_ioctl(dev, WLC_SET_AP, &apsta_var,
				   sizeof(apsta_var)))) {
		...
		res =
		    dev_wlc_ioctl(dev, WLC_GET_AP, &apsta_var,
				  sizeof(apsta_var));
#else

		apsta_var = 1;
		iolen = wl_bssiovar_mkbuf("apsta",
					  bsscfg_index, &apsta_var,
					  sizeof(apsta_var) + 4, buf,
					  sizeof(buf), &mkvar_err);
		ASSERT(iolen);
		...
		updown = 1;
		if ((res =
		     dev_wlc_ioctl(dev, WLC_UP, &updown, sizeof(updown))) < 0) {
			WL_ERROR(("%s fail to set apsta \n", __FUNCTION__));
			goto fail;
		}
	...
	iolen = wl_bssiovar_mkbuf("closednet",
				  bsscfg_index, &ap->closednet,
				  sizeof(ap->closednet) + 4, buf, sizeof(buf),
				  &mkvar_err);
	ASSERT(iolen);
	if ((res = dev_wlc_ioctl(dev, WLC_SET_VAR, buf, iolen)) < 0) {
	...
	iolen = wl_bssiovar_mkbuf("ssid", bsscfg_index, (char *)(&ap_ssid),
				  ap_ssid.SSID_len + 4, buf, sizeof(buf),
				  &mkvar_err);
	ASSERT(iolen);
	if ((res = dev_wlc_ioctl(dev, WLC_SET_VAR, buf, iolen)) != 0) {
...
static int wl_iw_set_ap_security(struct net_device *dev, struct ap_profile *ap)
{
	...
	if (strnicmp(ap->sec, "open", strlen("open")) == 0) {
		...
		if (key_len < WSEC_MAX_PSK_LEN) {
			...
			pbkdf2_sha1(ap->key, ap->ssid, strlen(ap->ssid), 4096,
				    output, 32);
...
static int
get_parameter_from_string(char **str_ptr, const char *token,
...
static int wl_iw_softap_deassoc_stations(struct net_device *dev, u8 *mac)
{
	...
	if (assoc_maclist->count)
		for (i = 0; i < assoc_maclist->count; i++) {
			...
			if (deauth_all
			    || (memcmp(&scbval.ea, sta_mac, ETHER_ADDR_LEN) ==
				0)) {
		   ...
		   union iwreq_data *wrqu, char *ext)
{
	...
	if ((ap_cfg_running == TRUE)) {
		...
		if ((res = dev_iw_write_cfg1_bss_var(dev, 2)) < 0)
			WL_ERROR(("%s failed to del BSS err = %d", __FUNCTION__,
				  res));
#endif
			...
			union iwreq_data *wrqu, char *ext)
{
	...
	if (wrqu->data.length != 0) {
		...
		if (copy_from_user
		    (params, wrqu->data.pointer, wrqu->data.length)) {
			kfree(params);
			return -EFAULT;
		}
...
static int
iwpriv_en_ap_bss(struct net_device *dev,
...
static int set_ap_mac_list(struct net_device *dev, void *buf)
{
	...
	if (mac_mode == MACLIST_MODE_DISABLED) {
		...
		ioc_res =
		    dev_wlc_ioctl(dev, WLC_SET_MACMODE, &mac_mode,
				  sizeof(mac_mode));
		...
		length =
		    sizeof(maclist->count) + maclist->count * ETHER_ADDR_LEN;
		...
		for (i = 0; i < maclist->count; i++)
			WL_SOFTAP(("mac %d: %02X:%02X:%02X:%02X:%02X:%02X\n",
				   i, maclist->ea[i].octet[0],
				   maclist->ea[i].octet[1],
				   maclist->ea[i].octet[2],
				   maclist->ea[i].octet[3],
				   maclist->ea[i].octet[4],
			...
			for (i = 0; i < assoc_maclist->count; i++) {
				...
				if (((mac_mode == MACLIST_MODE_ALLOW)
					...
					ioc_res = dev_wlc_ioctl(dev,
								...
								sizeof
								(scb_val_t));
					check_error(ioc_res, "ioctl ERROR:",
...
static int
wl_iw_process_private_ascii_cmd(struct net_device *dev,
				struct iw_request_info *info,
				union iwreq_data *dwrq, char *cmd_str)
{
	...
	if (strnicmp(sub_cmd, "AP_CFG", strlen("AP_CFG")) == 0) {
		...
		if (init_ap_profile_from_string
		    (cmd_str + PROFILE_OFFSET, &my_ap) != 0) {
	...
	} else if (strnicmp(sub_cmd, "AP_BSS_START", strlen("AP_BSS_START")) ==
		   0) {
		...
		if (ap_net_dev == NULL) {
			...
			if ((ret =
			     iwpriv_en_ap_bss(ap_net_dev, info, dwrq,
					      cmd_str)) < 0)
...
static int
wl_iw_set_priv(struct net_device *dev,
	       struct iw_request_info *info, struct iw_point *dwrq, char *ext)
{
	int ret = 0;
	char *extra;
	...
	if (dwrq->length && extra) {
		...
		if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
...
#else
			ret =
			    wl_iw_set_active_scan(dev, info,
						  (union iwreq_data *)dwrq,
						  extra);
#endif
		} else
		    if (strnicmp(extra, "SCAN-PASSIVE", strlen("SCAN-PASSIVE"))
			== 0)
#ifdef ENABLE_ACTIVE_PASSIVE_SCAN_SUPPRESS
...
#else
			ret =
			    wl_iw_set_passive_scan(dev, info,
						   (union iwreq_data *)dwrq,
						   extra);
#endif
		else if (strnicmp(extra, "RSSI", strlen("RSSI")) == 0)
			ret =
			    wl_iw_get_rssi(dev, info, (union iwreq_data *)dwrq,
					   extra);
		else if (strnicmp(extra, "LINKSPEED", strlen("LINKSPEED")) == 0)
			ret =
			    wl_iw_get_link_speed(dev, info,
						 (union iwreq_data *)dwrq,
						 extra);
			...
			ret =
			    wl_iw_set_country(dev, info,
					      (union iwreq_data *)dwrq, extra);
		else if (strnicmp(extra, "STOP", strlen("STOP")) == 0)
			ret = wl_iw_control_wl_off(dev, info);
		else if (strnicmp(extra, BAND_GET_CMD, strlen(BAND_GET_CMD)) ==
			 0)
			ret =
			    wl_iw_get_band(dev, info, (union iwreq_data *)dwrq,
					   extra);
		else if (strnicmp(extra, BAND_SET_CMD, strlen(BAND_SET_CMD)) ==
			 0)
			ret =
			    wl_iw_set_band(dev, info, (union iwreq_data *)dwrq,
					   extra);
		else if (strnicmp
			 (extra, DTIM_SKIP_GET_CMD,
			  strlen(DTIM_SKIP_GET_CMD)) == 0)
			ret =
			    wl_iw_get_dtim_skip(dev, info,
						(union iwreq_data *)dwrq,
						extra);
		else if (strnicmp
			 (extra, DTIM_SKIP_SET_CMD,
			  strlen(DTIM_SKIP_SET_CMD)) == 0)
			ret =
			    wl_iw_set_dtim_skip(dev, info,
						(union iwreq_data *)dwrq,
						extra);
		else if (strnicmp
			 (extra, SETSUSPENDOPT_CMD,
			  strlen(SETSUSPENDOPT_CMD)) == 0)
			ret =
			    wl_iw_set_suspend_opt(dev, info,
						  (union iwreq_data *)dwrq,
						  extra);
		else if (strnicmp
			 (extra, SETSUSPENDMODE_CMD,
			  strlen(SETSUSPENDMODE_CMD)) == 0)
			ret =
			    wl_iw_set_suspend_mode(dev, info,
						   (union iwreq_data *)dwrq,
						   extra);
		else if (strnicmp
			 (extra, TXPOWER_SET_CMD, strlen(TXPOWER_SET_CMD)) == 0)
			ret =
			    wl_iw_set_txpower(dev, info,
					      (union iwreq_data *)dwrq, extra);
#if defined(PNO_SUPPORT)
		else if (strnicmp
			 (extra, PNOSSIDCLR_SET_CMD,
			  strlen(PNOSSIDCLR_SET_CMD)) == 0)
			ret =
			    wl_iw_set_pno_reset(dev, info,
						(union iwreq_data *)dwrq,
						extra);
		else if (strnicmp
			 (extra, PNOSETUP_SET_CMD,
			  strlen(PNOSETUP_SET_CMD)) == 0)
			ret =
			    wl_iw_set_pno_set(dev, info,
					      (union iwreq_data *)dwrq, extra);
		else if (strnicmp
			 (extra, PNOSETADD_SET_CMD,
			  strlen(PNOSETADD_SET_CMD)) == 0)
			ret =
			    wl_iw_set_pno_setadd(dev, info,
						 (union iwreq_data *)dwrq,
						 extra);
		else if (strnicmp
			 (extra, PNOENABLE_SET_CMD,
			  strlen(PNOENABLE_SET_CMD)) == 0)
			ret =
			    wl_iw_set_pno_enable(dev, info,
						 (union iwreq_data *)dwrq,
						 extra);
#endif
#if defined(CSCAN)

		else if (strnicmp(extra, CSCAN_COMMAND, strlen(CSCAN_COMMAND))
			 == 0)
			ret =
			    wl_iw_set_cscan(dev, info, (union iwreq_data *)dwrq,
					    extra);
#endif
#ifdef CONFIG_WPS2
			...
			ret = wl_iw_add_wps_probe_req_ie(dev, info,
			 ...
			 (extra, WPS_DEL_PROBE_REQ_IE_CMD,
			  strlen(WPS_DEL_PROBE_REQ_IE_CMD)) == 0)
			ret =
			    wl_iw_del_wps_probe_req_ie(dev, info,
						       (union iwreq_data *)dwrq,
						       extra);
#endif
		else if (strnicmp(extra, "POWERMODE", strlen("POWERMODE")) == 0)
			ret =
			    wl_iw_set_power_mode(dev, info,
						 (union iwreq_data *)dwrq,
						 extra);
		else if (strnicmp(extra, "BTCOEXMODE", strlen("BTCOEXMODE")) ==
			 0)
			ret =
			    wl_iw_set_btcoex_dhcp(dev, info,
						  (union iwreq_data *)dwrq,
						  extra);
		else if (strnicmp(extra, "GETPOWER", strlen("GETPOWER")) == 0)
			ret =
			    wl_iw_get_power_mode(dev, info,
						 (union iwreq_data *)dwrq,
						 extra);
#ifdef SOFTAP
		else if (strnicmp(extra, "ASCII_CMD", strlen("ASCII_CMD")) == 0) {
			wl_iw_process_private_ascii_cmd(dev, info,
							(union iwreq_data *)
							dwrq, extra);
		} else
		    if (strnicmp
			(extra, "AP_MAC_LIST_SET",
			 strlen("AP_MAC_LIST_SET")) == 0) {
...
void wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void *data)
{
		...
		if (ap_cfg_running) {
			...
			if (ap_macmode) {
				...
				if (i == ap_black_list.count) {
					...
					char mac_buf[32] = { 0 };
					sprintf(mac_buf,
						"STA_BLOCK %02X:%02X:%02X:%02X:%02X:%02X",
						macaddr[0], macaddr[1],
						macaddr[2], macaddr[3],
						macaddr[4], macaddr[5]);
					wl_iw_send_priv_event(priv_dev,
							      mac_buf);
				}
			}
		}
	...
	case WLC_E_NDIS_LINK:
		...
		if (!(flags & WLC_EVENT_MSG_LINK)) {
			...
			memcpy(g_ss_cache_ctrl.m_active_bssid, &e->addr,
			       ETHER_ADDR_LEN);
#ifdef SOFTAP
	...
	case WLC_E_PFN_NET_FOUND:
		{
			...
			netinfo =
			    (wl_pfn_net_info_t *) (data +
						   sizeof(wl_pfn_scanresults_t)
						   - sizeof(wl_pfn_net_info_t));
			WL_ERROR(("%s Event WLC_E_PFN_NET_FOUND, send %s up : find %s len=%d\n", __FUNCTION__, PNO_EVENT_UP, netinfo->pfnsubnet.SSID, netinfo->pfnsubnet.SSID_len));
			cmd = IWEVCUSTOM;
			memset(&wrqu, 0, sizeof(wrqu));
...
static int _bt_dhcp_sysioc_thread(void *data)
{
	tsk_ctl_t *tsk_ctl = (tsk_ctl_t *) data;
	...
	while (down_interruptible(&tsk_ctl->sema) == 0) {
		...
		case BT_DHCP_OPPORTUNITY_WINDOW:
			...
			WL_TRACE_COEX(("%s DHCP T1:%d expired\n",
				       __FUNCTION__,
				       BT_DHCP_OPPORTUNITY_WINDOW_TIME));

			if (g_bt->dev)
				wl_iw_bt_flag_set(g_bt->dev, TRUE);
			g_bt->bt_state = BT_DHCP_FLAG_FORCE_TIMEOUT;
			mod_timer(&g_bt->timer,
				  jiffies +
				  BT_DHCP_FLAG_FORCE_TIME * HZ / 1000);
			g_bt->timer_on = 1;
			break;
...
static int wl_iw_bt_init(struct net_device *dev)
{
