...
struct dhd_cmn *dhd_common_init(osl_t *osh)
{
	...
	bcm_strncpy_s(fw_path, sizeof(fw_path), CONFIG_BCMDHD_FW_PATH,
		      MOD_PARAM_PATHLEN - 1);
#else /* CONFIG_BCMDHD_FW_PATH */
	fw_path[0] = '\0';
#endif /* CONFIG_BCMDHD_FW_PATH */
#ifdef CONFIG_BCMDHD_NVRAM_PATH
	bcm_strncpy_s(nv_path, sizeof(nv_path), CONFIG_BCMDHD_NVRAM_PATH,
		      MOD_PARAM_PATHLEN - 1);
...
int
dhd_wl_ioctl_cmd(dhd_pub_t *dhd_pub, int cmd, void *arg, int len, uint8 set,
		 int ifindex)
{
	wl_ioctl_t ioc;
...
int
dhd_wl_ioctl(dhd_pub_t *dhd_pub, int ifindex, wl_ioctl_t *ioc, void *buf,
	     int len)
{
	int ret;
	    ...
	    int val_size)
{
	...
	case IOV_SVAL(IOV_HCI_ACL_DATA):{
			...
			if (len <
			    (int)(HCI_ACL_DATA_PREAMBLE_SIZE + ACL_data->dlen))
				return BCME_BUFTOOSHORT;
...
static void wl_show_host_event(wl_event_msg_t *event, void *event_data)
{
	...
	default:
		DHD_EVENT(("MACEVENT: %s %d, MAC %s, status %d, reason %d, auth %d\n", event_name, event_type, eabuf, (int)status, (int)reason, (int)auth_type));
		break;
	}
	...
	case WLC_E_IF:
		{
...
#ifdef PROP_TXSTATUS
			{
				...
				WLFC_DBGMESG(("WLC_E_IF: idx:%d, action:%s, iftype:%s, " "[%02x:%02x:%02x:%02x:%02x:%02x]\n", ifevent->ifidx, ((ifevent->action == WLC_E_IF_ADD) ? "ADD" : "DEL"), ((ifevent->is_AP == 0) ? "STA" : "AP "), ea[0], ea[1], ea[2], ea[3], ea[4], ea[5]));
				(void)ea;
			...
			if (ifevent->ifidx > 0 && ifevent->ifidx < DHD_MAX_IFS) {
				if (ifevent->action == WLC_E_IF_ADD) {
					if (dhd_add_if
					    (dhd_pub->info, ifevent->ifidx,
					     NULL, event->ifname,
					     event->addr.octet, ifevent->flags,
					     ifevent->bssidx)) {
						DHD_ERROR(("%s: dhd_add_if failed!!" " ifidx: %d for %s\n", __FUNCTION__, ifevent->ifidx, event->ifname));
						return BCME_ERROR;
					}
				} else
					dhd_del_if(dhd_pub->info,
						   ifevent->ifidx);
			} else {
...
void dhd_aoe_arp_clr(dhd_pub_t *dhd)
{
	...
	if ((ret =
	     dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, iov_len, TRUE, 0) < 0))
...
void dhd_arp_offload_add_ip(dhd_pub_t *dhd, uint32 ipaddr)
{
	...
	iov_len =
	    bcm_mkiovar("arp_hostip", (char *)&ipaddr, 4, iovbuf,
			sizeof(iovbuf));
...
static int dhd_iscan_get_partial_result(void *dhdp, uint *scan_count)
{
	...
	ioctl.set = FALSE;
	rc = dhd_wl_ioctl(dhd, 0, &ioctl, iscan_cur->iscan_buf,
			  WLC_IW_ISCAN_MAXLEN);
...
int dhd_get_dtim_skip(dhd_pub_t *dhd)
{
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_DTIMPRD,
				    &dtim_assoc, sizeof(dtim_assoc), FALSE,
				    0)) < 0) {
		DHD_ERROR(("%s failed code %d\n", __FUNCTION__, ret));
		goto exit;
	}
...
int
dhd_pno_set(dhd_pub_t *dhd, wlc_ssid_t *ssids_local, int nssid,
	...
	while (*bytes_left > 0) {
		...
		*bytes_left -= 1;
		...
		if (input_size == 1) {
			...
			memcpy(dst,
			       (char *)
			       htod16(memcpy(&short_temp, str, input_size)),
			       input_size);
		} else if (input_size == 4) {
			memcpy(dst,
			       (char *)
			       htod32(memcpy(&int_temp, str, input_size)),
			       input_size);
		}
 ...
 */
int
wl_iw_parse_ssid_list_tlv(char **list_str, wlc_ssid_t *ssid, int max,
			  int *bytes_left)
{
	char *str;
	int idx = 0;
...
int wl_iw_parse_ssid_list(char **list_str, wlc_ssid_t *ssid, int idx, int max)
{
	...
	for (str = *list_str; str != NULL; str = ptr) {
		...
		if (idx < max) {
			bcm_strcpy_s((char *)ssid[idx].SSID,
				     sizeof(ssid[idx].SSID), str);
 ...
 */
int
wl_iw_parse_channel_list(char **list_str, uint16 *channel_list,
			 int channel_num)
{
	int num;
	int val;
