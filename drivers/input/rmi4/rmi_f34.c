...
static int rmi_f34_flash_firmware(struct rmi_function_container *fc);
...
static int rmi_f34_init(struct rmi_function_container *fc)
{
	...
	retval = rmi_f34_flash_firmware(fc);
	if (retval < 0)
		goto exit_free_data;
...
static int rmi_f34_initialize(struct rmi_function_container *fc)
{
	...
	/* Get custom config id and store it in the instance data */
	retval = rmi_read_block(fc->rmi_dev, control_base_addr, f34->id,
			ARRAY_SIZE(f34->id));

	if (retval < 0) {
		dev_err(&fc->dev, "Couldn't read custom config id from 0x%x, "
			"error=%d.\n", control_base_addr, retval);
		return retval;
	} else {
		dev_info(&fc->dev, "firmware id = %02x %02x %02x %02x.\n",
			f34->id[0], f34->id[1], f34->id[2], f34->id[3]);
	}

	return 0;
}

static int rmi_f34_rescan_pdt(struct rmi_function_container *fc)
{
	int i, retval;

	/* Try several time since touch ic may stop
	 * response a short period during flashing */
	for (i = 0; i < RETRY_COUNT; i++) {
		retval = rmi_fn_34_rescanPDT_store(&fc->dev, NULL, "1", 2);
		if (retval >= 0)
			break;
		msleep(RETRY_SLEEP);
	}
...
static int rmi_f34_send_command(struct rmi_function_container *fc,
				u8 command, u8 *data, int size)
{
	struct rmi_fn_34_data *f34 = fc->data;
	int i, j, bytes, retval = 0;
	unsigned short block = 0;
	unsigned char status;

	/* Step 1: write block number */
	retval = rmi_write_block(fc->rmi_dev,
			fc->fd.data_base_addr, (u8 *)&block, 2);
	if (retval < 0) {
		dev_err(&fc->dev,
			"%s: Write block number failed(%02x)\n",
			__func__, command);
		return retval;
	}
	...
	for (i = 0; i < size; i += bytes) {
		...
		bytes = min(size - i, (int)f34->blocksize);
		retval = rmi_write_block(fc->rmi_dev,
				fc->fd.data_base_addr + 2, &data[i], bytes);
		if (retval < 0) {
			dev_err(&fc->dev,
				"%s: Write command data failed(%02x)\n",
				__func__, command);
			return retval;
		}
		...
		retval = rmi_write_block(fc->rmi_dev,
				fc->fd.data_base_addr + 2 + f34->blocksize,
				&command, 1);
		if (retval < 0) {
			dev_err(&fc->dev,
				"%s: Write command failed(%2d)\n",
				__func__, command);
			return retval;
		}

		/* Step 4: rescan pdt for ENABLE_FLASH_PROG */
		if (command == ENABLE_FLASH_PROG) {
			retval = rmi_f34_rescan_pdt(fc);
			if (retval < 0) {
				dev_err(&fc->dev,
					"%s: Rescan PDT failed(%02x)\n",
					__func__, command);
				return retval;
			}
		}
		...
		for (j = 0; j < RETRY_COUNT; j++) {
			...
			retval = rmi_read_block(fc->rmi_dev,
					fc->fd.data_base_addr + 2 + f34->blocksize,
					&status, 1);
			if (retval >= 0 && !(status & 0x0f))
				break;
			msleep(RETRY_SLEEP);
		}
		if (retval < 0) {
			dev_err(&fc->dev,
				"%s: Read status failed(%02x)\n",
				__func__, command);
			return retval;
		}
		if (status & 0x7f) {
			dev_err(&fc->dev,
				"%s: Execute command failed(%02x, %02x)\n",
				__func__, command, status);
			return -EFAULT;
		}
	}
...
static int rmi_f34_reset_device(struct rmi_function_container *fc)
{
	int retval;

	retval = do_initial_reset(fc->rmi_dev);
	if (retval < 0) {
		dev_err(&fc->dev, "%s: Reset device failed\n", __func__);
		return retval;
	}

	retval = rmi_f34_rescan_pdt(fc);
	if (retval < 0) {
		dev_err(&fc->dev, "%s: Rescan PDT failed\n", __func__);
		return retval;
	}
...
static int rmi_f34_flash_firmware(struct rmi_function_container *fc)
{
	...
	struct rmi_f34_firmware *firmware;
	struct rmi_firmware_header header;
	u8 bootloaderid[2];
	int retval = 0;
	...
	/* Directly return if id is same */
	if (!memcmp(f34->id, firmware->id, sizeof(firmware->id)))
		return retval;

	dev_info(&fc->dev,
		"Flash firmware(%02x %02x %02x %02x)\n",
		firmware->id[0], firmware->id[1],
		firmware->id[2], firmware->id[3]);

	hstoba(bootloaderid, f34->bootloaderid);

	header = *(struct rmi_firmware_header *)firmware->data;
	...
	disable_irq(gpio_to_irq(pdata->attn_gpio));

	retval = rmi_f34_send_command(fc, ENABLE_FLASH_PROG,
			bootloaderid, sizeof(bootloaderid));
	if (retval < 0)
		goto exit;

	retval = rmi_f34_send_command(fc, ERASE_ALL,
			bootloaderid, sizeof(bootloaderid));
	if (retval < 0)
		goto exit;

	retval = rmi_f34_send_command(fc, WRITE_FW_BLOCK,
			firmware->data + sizeof(header),
			header.firmware_size);
	if (retval < 0)
		goto exit;

	retval = rmi_f34_send_command(fc, WRITE_CONFIG_BLOCK,
			firmware->data + sizeof(header) + header.firmware_size,
			header.config_size);
	if (retval < 0)
		goto exit;

	retval = rmi_f34_reset_device(fc);
	if (retval < 0)
		goto exit;

exit:
	enable_irq(gpio_to_irq(pdata->attn_gpio));
	return retval;
}
