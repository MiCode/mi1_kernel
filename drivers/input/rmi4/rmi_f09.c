...
static struct device_attribute attrs[] = {
	...
	__ATTR(hostTestEnable, RMI_RO_ATTR,
	       rmi_f09_host_test_enable_show, rmi_store_error),
...
static int rmi_f09_initialize(struct rmi_function_container *fc)
{
	...
	rc = rmi_read_block(rmi_dev, fc->fd.control_base_addr,
		(u8 *)&f09->control, sizeof(f09->control));
	if (rc < 0) {
		dev_err(&fc->dev, "Failed to read control register."
...
static int rmi_f09_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	error = rmi_read_block(rmi_dev, fc->fd.data_base_addr,
		(u8 *)&data->data, sizeof(data->data));
	if (error < 0) {
		dev_err(&fc->dev, "Failed to read data register."
					...
					const char *buf, size_t count)
{
	...
	if (new_low < 0 || new_low > 255 || new_high < 0 || new_high > 255
			|| new_diff < 0 || new_diff > 255) {
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr + 1,
		data->control.test1_limit_high);
	if (result < 0) {
		dev_err(dev, "%s : Could not write f09_control_test1_limit_high to 0x%x\n",
				__func__, fc->fd.control_base_addr + 1);
		return result;
	}

	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr + 2,
		data->control.test1_limit_diff);
	if (result < 0) {
		dev_err(dev, "%s : Could not write f09_control_test1_limit_diff to 0x%x\n",
				__func__, fc->fd.control_base_addr + 2);
		return result;
	}
					...
					const char *buf, size_t count)
{
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr + 3,
		data->control.test2_limit_low);
	if (result < 0) {
		dev_err(dev, "%s : Could not write f09_control_test2_limit_low to 0x%x\n",
				__func__, fc->fd.control_base_addr + 3);
		return result;
	}

	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr + 4,
		data->control.test2_limit_high);
	if (result < 0) {
		dev_err(dev, "%s : Could not write f09_control_test2_limit_high to 0x%x\n",
				__func__, fc->fd.control_base_addr + 4);
		return result;
	}

	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr + 5,
		data->control.test2_limit_diff);
	if (result < 0) {
		dev_err(dev, "%s : Could not write f09_control_test2_limit_diff to 0x%x\n",
				__func__, fc->fd.control_base_addr + 5);
		return result;
	}
					...
					const char *buf, size_t count)
{
	...
	result = rmi_write(fc->rmi_dev, fc->fd.data_base_addr,
		data->data.test_number_control);
	if (result < 0) {
