...
#define RMI_54_STOP_ANTITOUCH_TIME	(5*HZ)
...
struct rmi_fn_54_data {
	...
	struct work_struct force_calibration_work;
...
/* here we register a dummy device for receive msg from Eventhub Sensor info */
static int rmi_f54_register_device(struct rmi_function_container *fc);

static void rmi_f54_free_device(struct rmi_function_container *fc);

static void rmi_f54_force_calibration_work(struct work_struct *work);

static void rmi_f54_disable_antitouch_delayed_work(struct work_struct *work);

static void rmi_f54_enable_disable_antitouch(struct rmi_fn_54_data *f54,
			bool on);

#define BIT_ENERGY_RATIO_RELAXATION_ENABLE	5
static void rmi_f54_enable_disable_antitouch(struct rmi_fn_54_data *f54,
			bool on)
{
	struct rmi_function_container *fc = f54->fc;
	int result = 0;
	u8 f54_analog_control_0;

	result = rmi_read(fc->rmi_dev, fc->fd.control_base_addr,
			&f54_analog_control_0);
	if (result < 0) {
		dev_err(&fc->dev, "%s: Could not read control from 0x%x\n",
			__func__, fc->fd.control_base_addr);
		return;
	}

	if (on)
		u8_set_bit(&f54_analog_control_0,
			BIT_ENERGY_RATIO_RELAXATION_ENABLE);
	else
		u8_clear_bit(&f54_analog_control_0,
			BIT_ENERGY_RATIO_RELAXATION_ENABLE);

	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
			f54_analog_control_0);
	if (result < 0) {
		dev_err(&fc->dev, "%s: Could not write control to 0x%x\n",
			__func__, fc->fd.control_base_addr);
	}

	dev_info(&fc->dev, "current f54_analog_control_0 = %#04x",
		f54_analog_control_0);
}

#ifdef CONFIG_PM
static int rmi_f54_suspend(struct rmi_function_container *fc)
{
	struct rmi_fn_54_data *f54 = fc->data;

	/* set the BIT_ENERGY_RATIO_RELAXATION_ENABLE */
	dev_info(&fc->dev, "set the BIT_ENERGY_RATIO_RELAXATION_ENABLE\n");
	rmi_f54_enable_disable_antitouch(f54, true);

	return 0;
}

static int rmi_f54_resume(struct rmi_function_container *fc)
{
	struct rmi_fn_54_data *f54 = fc->data;

	dev_info(&fc->dev,
		"schedule the delayed work to use antitouch for 5 seconds\n");

	schedule_delayed_work(&f54->delayed_work, RMI_54_STOP_ANTITOUCH_TIME);
...
static void rmi_f54_disable_antitouch_delayed_work(struct work_struct *work)
{
	...
	struct rmi_fn_54_data *f54 = container_of(delayed_work,
			struct rmi_fn_54_data, delayed_work);
	struct rmi_function_container *fc = f54->fc;

	dev_info(&fc->dev, "disable anti-touch setting delayed work\n");

	rmi_f54_enable_disable_antitouch(f54, false);
}

static void rmi_f54_force_calibration_work(struct work_struct *work)
{
	struct rmi_fn_54_data *f54 = container_of(work,
			struct rmi_fn_54_data, force_calibration_work);
	struct rmi_function_container *fc = f54->fc;
	int result;
	u8 command;

	dev_info(&fc->dev, "Do force calib work!\n");

	command = (u8) FORCE_CAL;
	result = rmi_write_block(fc->rmi_dev, fc->fd.command_base_addr,
				&command, 1);
	if (result < 0) {
		dev_err(&fc->dev, "%s: Could not write command to 0x%x\n",
			__func__, fc->fd.command_base_addr);
	}
}

static int rmi_f54_input_event(struct input_dev *dev,
		unsigned int type, unsigned int code, int value)
{
	struct rmi_fn_54_data *f54 = input_get_drvdata(dev);
	struct rmi_function_container *fc = f54->fc;

	dev_info(&fc->dev, "receive type = %d code = %d\n", type, code);
	if (type == EV_SYN && code == SYN_CONFIG) {
		dev_info(&fc->dev, "Schedule force calib work!\n");
		schedule_work(&(f54->force_calibration_work));
	}
...
static int rmi_f54_register_device(struct rmi_function_container *fc)
{
	...
	f54->input_dev = input_dev;
	input_dev->name = "sensor00fn54";
	input_dev->phys = "sensor00fn54/input0";
	input_dev->dev.parent = &rmi_dev->dev;
	input_dev->event = rmi_f54_input_event;
	input_set_drvdata(input_dev, f54);

	set_bit(EV_SYN, input_dev->evbit);
	set_bit(EV_ABS, input_dev->evbit);
	set_bit(EV_KEY, input_dev->evbit);
	set_bit(BTN_TOUCH, input_dev->keybit);
	input_set_abs_params(input_dev, ABS_X, 0, 0, 0, 0);
	input_set_abs_params(input_dev, ABS_Y, 0, 0, 0, 0);

	rc = input_register_device(input_dev);
	if (!rc) {
		INIT_WORK(&f54->force_calibration_work,
			rmi_f54_force_calibration_work);
		INIT_DELAYED_WORK(&f54->delayed_work,
			rmi_f54_disable_antitouch_delayed_work);
		return 0;
	}
err_free_mem:
...
static void rmi_f54_free_device(struct rmi_function_container *fc)
{
	...
	cancel_work_sync(&f54->force_calibration_work);
	cancel_delayed_work_sync(&f54->delayed_work);

	input_unregister_device(f54->input_dev);
}
...
static int rmi_f54_init(struct rmi_function_container *fc)
{
	...
	retval = rmi_f54_register_device(fc);
	if (retval < 0)
		goto error_exit;
...
int rmi_f54_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	/* 128 bytes per transfer to avoid some bus controller limitation */
	for (l = 0; l < data->report_size; l += 128) {
		/* Write offset to fifohi and fifolo. */
		fifo[0] = l;
		fifo[1] = (l >> 8);
		error = rmi_write_block(fc->rmi_dev, fc->fd.data_base_addr
					+ RMI_F54_FIFO_OFFSET, fifo,	sizeof(fifo));
		if (error < 0)
			dev_err(&fc->dev, "Failed to write fifo to offset!\n");
		else
			error = rmi_read_block(fc->rmi_dev,
				fc->fd.data_base_addr + RMI_F54_REPORT_DATA_OFFSET,
				data->report_data + l, min(data->report_size - l, 128u));
		if (error < 0) {
